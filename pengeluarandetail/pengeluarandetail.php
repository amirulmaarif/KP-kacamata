...
<hr>
<div id="accordion">
	<h3>Masukan Detail Pengeluaran </h3>
	<div>
...
<th colspan="2"><select name="id_produk" class="form-control" style="width: 550px;">
	...
	<tr>
		...
		<td>:
		<td><input required class="form-control" style="width: 150px;" name="jumlah" type="number" id="jumlah" min="1"  value="<?php echo $jumlah; ?>" size="25" /></td>
	</tr>
	...
	<tr>
		...
		<td colspan="2">
			...
			<a href="?mnu=pengeluarandetail&id=<?php echo $id_pengeluaran; ?>"><input name="Batal" class="btn btn-danger" type="button" id="Batal" value="Batal" /></a>
		</td>
	</tr>
</table>
</form>
...
<font color="black">Data pengeluaran Detail</font> | <img src='ypathicon/print.png' title='PRINT' OnClick="PRINT('<?php echo $id_pengeluaran; ?>')"> |
...
<img src='$YPATH/$gambar' width='40' height='40' /></a></div>";
				...
				<td><small>$catatan</td>
				<td><div align='center'>	
<a href='?mnu=pengeluarandetail&id=$id_pengeluaran&pro=ubah&kode=$id_td'><img src='ypathicon/ub.png' title='ubah'></a>
<a href='?mnu=pengeluarandetail&id=$id_pengeluaran&pro=hapus&kode=$id_td&nama_produk=$nama_produk&id_produk=$id_produk&jumlah=$jumlah'><img src='ypathicon/ha.png' title='hapus' 
...
if (isset($_POST["Simpan"])) {
	...
	$sqlc="select  * from `$tbproduk` where `id_produk`='$id_produk'";
	 $dc=getField($conn,$sqlc);
				...
				$stok = $dc["stok"];
					$produk = $dc["nama_produk"];
	
if($stok<$jumlah){
				echo "<script>alert('Data $produk Gagal Order, Stok Kurang !');
				document.location.href='?mnu=pengeluarandetail&id=$id_pengeluaran&id=$id_pengeluaran';</script>";
...
'$catatan'
		...
		if ($simpan) {
			echo "<script>alert('Data $produk berhasil disimpan !');document.location.href='?mnu=pengeluarandetail&id=$id_pengeluaran&id=$id_pengeluaran';</script>";
		} else {
			echo "<script>alert('Data $produk gagal disimpan...');document.location.href='?mnu=pengeluarandetail&id=$id_pengeluaran&id=$id_pengeluaran';</script>";
		}
	} else {
	...
	`catatan`='$catatan'
	 where `id_td`='$id_td0'";
		...
		if ($ubah) {
			echo "<script>alert('Data $produk berhasil diubah !');document.location.href='?mnu=pengeluarandetail&id=$id_pengeluaran&id=$id_pengeluaran';</script>";
		} else {
			echo "<script>alert('Data $produk gagal diubah...');document.location.href='?mnu=pengeluarandetail&id=$id_pengeluaran&id=$id_pengeluaran';</script>";
		}
	} //else simpan
}
}
...
if ($_GET["pro"] == "hapus") {
	...
	if ($hapus) {
		echo "<script>alert('Data $nama_produk berhasil dihapus !');document.location.href='?mnu=pengeluarandetail&id=$id_pengeluaran&id=$id_pengeluaran';</script>";
	} else {
		echo "<script>alert('Data $nama_produk gagal dihapus...');document.location.href='?mnu=pengeluarandetail&id=$id_pengeluaran&id_pengeluaran=$id_pengeluaran';</script>";
	}
}
?>